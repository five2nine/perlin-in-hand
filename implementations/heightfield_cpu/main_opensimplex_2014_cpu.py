...
def compute_z(
    ...
    for row in range(Z.shape[0]):
        for col in range(Z.shape[1]):
            # Using 3D OpenSimplex noise: x, y, and t (z axis for time)
            Z[row, col] = (
                generator.noise3(
...
def benchmark_compute(
    ...
    for tidx in tqdm(range(frame_count), desc="Computing OpenSimplex noise", unit="frame"):
...
def _draw_surface(
    ...
    ax.set_ylabel("y")
    ax.set_zlabel("noise")
    ax.set_title(f"OpenSimplex (CPU) | t={t_phase:.3f}")
...
def main() -> None:
    frame_count = 60 * 5
    generator = opensimplex.OpenSimplex(seed=6789)
