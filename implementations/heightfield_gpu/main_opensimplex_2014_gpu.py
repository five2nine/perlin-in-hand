...
with open(os.path.join(SCRIPT_DIR, "shaders", "opensimplex_2014.vert"), "r", encoding="utf-8") as f:
    VERTEX_SHADER = f.read()
with open(os.path.join(SCRIPT_DIR, "shaders", "opensimplex_2014.frag"), "r", encoding="utf-8") as f:
    FRAGMENT_SHADER = f.read()


class OpenSimplexNoise3DApp(mglw.WindowConfig):
    gl_version = (3, 3)
    title = "Real-Time 3D OpenSimplex Noise Terrain"
    ...
    def __init__(self, **kwargs):
        ...
        print("=" * 60)
        print("Interactive 3D OpenSimplex Noise Terrain (ModernGL)")
        print("-" * 60)
        print("Controls:")
    ...
    def on_render(self, time_since_start: float, frametime: float):
        ...
        if self.fps_timer >= 0.5:
            ...
            self.wnd.title = (
                f"3D OpenSimplex Noise | Resolution: {self.resolution}x{self.resolution} | "
...
def main() -> None:
    mglw.run_window_config(OpenSimplexNoise3DApp)
