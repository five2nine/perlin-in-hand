# Static heightfield terrain generator with a layer-stack UI.
...
with open(
    os.path.join(SCRIPT_DIR, "shaders", "terrain_bake.vert"),
    "r",
    encoding="utf-8",
) as f:
    BAKE_VERTEX_SHADER = f.read()

with open(
    os.path.join(SCRIPT_DIR, "shaders", "terrain_heightfield.vert"),
    "r",
    encoding="utf-8",
) as f:
...
with open(
    os.path.join(SCRIPT_DIR, "shaders", "terrain_heightfield.frag"),
    "r",
    encoding="utf-8",
) as f:
...
class GPUTerrainGeneratorApp(mglw.WindowConfig):
    ...
    def __init__(self, **kwargs):
        ...
        self.bake_prog = self.ctx.program(
            vertex_shader=BAKE_VERTEX_SHADER,
    ...
    def _get_uniform(self, program, name: str):
    ...
    def _set_uniform_value(self, program, name: str, value) -> None:
    ...
    def rebuild_grid(self) -> None:
        ...
        if self.bake_vao:
            self.bake_vao.release()
        if self.render_vao:
            self.render_vao.release()

        self.uv_vbo = self.ctx.buffer(vertices.tobytes())
        self.ebo = self.ctx.buffer(index_data.tobytes())
        self.terrain_vbo = self.ctx.buffer(reserve=vertices.shape[0] * 7 * 4)
        self.bake_vao = self.ctx.vertex_array(
            self.bake_prog,
            [(self.uv_vbo, "2f", "in_vert")],
        )
        ...
        self._set_uniform_value(self.bake_prog, "u_grid_size", int(self.resolution))
        self.bake_terrain()

    def upload_terrain_stack(self) -> None:
        packed = self.stack.pack_gpu()
        self._set_uniform_value(self.bake_prog, "u_layer_count", int(packed["count"]))
        ...
        self._set_uniform_value(
            self.render_prog,
            "u_height_color_scale",
            float(self.stack.height_color_scale()),
        )
    ...
    def on_render(self, time_since_start: float, frametime: float):
        ...
        if self.fps_timer >= 0.5:
            ...
            self.wnd.title = (
                f"GPU Terrain | Grid: {self.resolution}x{self.resolution} | "
...
def main() -> None:
    mglw.run_window_config(GPUTerrainGeneratorApp)
