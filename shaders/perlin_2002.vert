...
// --- Stefan Gustavson's Classic 3D Noise ---
...
float get_height(vec2 uv) {
    return cnoise(vec3(uv * u_frequency, u_time)) * u_amplitude;
}
...
void main() {
    ...
    // Finite difference vectors for normal calculation
    // tx is tangent along X
    // tz is tangent along Z (using the y grid coordinate)
    vec3 tx = vec3(eps, hx - h0, 0.0);
    vec3 tz = vec3(0.0, hy - h0, eps);
    vec3 normal = normalize(cross(tz, tx)); // points upwards (positive Y)
